    ...
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            ...
            if (response.success) {
                ...
                const transactionList = document.querySelector(".transactions");
                const originalTransaction = document.querySelector(`.transaction[data-id="${transactionId}"]`);
                const newTransaction = originalTransaction.cloneNode(true);
                newTransaction.setAttribute("data-id", response.transaction_id);
                transactionList.appendChild(newTransaction);
                //alert("Transaction duplicated successfully!");
                //console.log(response);
            } else {
                alert("Error duplicating transaction: " + response.message);
            }
        }
    }
    xhttp.open("POST", "transaction_duplicate.php", true);
    xhttp.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
    xhttp.send(`transaction_id=${transactionId}`);
}

function filterTransactionsByTicker(ticker) {
    const xhttp = new XMLHttpRequest();
    console.log("filter transaction by ticker:", ticker);
    xhttp.onreadystatechange = function() {
        if (this.readyState == 4 && this.status == 200) {
            document.querySelector(".transactions").innerHTML = this.responseText;
        }
